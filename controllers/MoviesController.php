...
class MoviesController
{
    ...
    public function khuyenmai()
    {
        ...
        $heroStats = [
            ['label' => 'Mã đang hoạt động', 'value' => (string)($stats['active'] ?? 0)],
            ['label' => 'Mã sắp tới', 'value' => (string)($stats['upcoming'] ?? 0)],
            ['label' => 'Tổng mã khuyến mãi', 'value' => (string)(($stats['active'] ?? 0) + ($stats['upcoming'] ?? 0))]
        ];

        // Kết hợp promotions từ discounts đã được format
        $allPromotions = $discounts;
