...
class MoviesController
{
    ...
    public function khuyenmai()
    {
        ...
        foreach ($discountCodes as $dc) {
            if ($dc['status'] === 'active') {
                ...
                $usageLimit = $dc['usage_limit'] ?? null;
                $usageUsed = $dc['usage_used'] ?? 0;
                $remainingUses = $usageLimit !== null ? max(0, (int)$usageLimit - (int)$usageUsed) : null;
                $usageLabel = $usageLimit !== null
                    ? 'Còn ' . $remainingUses . '/' . $usageLimit . ' lượt'
                    : 'Không giới hạn lượt';
    ...
    public function checkVoucher()
    {
        ...
        $seatCount = isset($_GET['seat_count']) ? (int)$_GET['seat_count'] : 1;
    ...
    public function movieDetail()
    {
        ...
        // Giới hạn chỉ hiển thị 7 ngày từ startDate
        $endDate = date('Y-m-d', strtotime($startDate . ' +6 days'));

        // Tạo danh sách ngày (chỉ 7 ngày)
        $dates = [];
        $currentDate = strtotime($startDate);
        $endTimestamp = strtotime($endDate);
        $dayCount = 0;

        while ($currentDate <= $endTimestamp && $dayCount < 7) {
