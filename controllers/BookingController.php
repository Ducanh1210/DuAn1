...
class BookingController
{
    ...
    /**
 * Chọn rạp (nếu có nhiều rạp/phòng cùng phim/ngày)
 */
public function selectCinema()
{
    ...
    // lấy các showtimes khác cùng movie cùng ngày (các rạp/room khả dụng)
    $cinemas = $this->showtime->getCinemasForShowtime($showtimeId);

    // Nếu chỉ có 1 rạp/phòng => tự động chuyển thẳng tới chọn ghế
    if (is_array($cinemas) && count($cinemas) === 1) {
        $s = $cinemas[0];
        // redirect tới trang chọn ghế (act=datve) với showtime_id tương ứng
        header('Location: ' . BASE_URL . '?act=datve&showtime_id=' . urlencode($s['showtime_id']));
        exit;
    }

    // Nếu có nhiều hơn 1, hiển thị trang chọn rạp
    renderClient('client/select_cinema.php', [
        'showtime' => $showtime,
        'movie' => $movie,
        'cinemas' => $cinemas
    ], 'Chọn rạp - ' . htmlspecialchars($movie['title']));
}
