...
class BookingController
{
    ...
    public function processPayment()
    {
        ...
        try {
            ...
            $seatCount = max(1, count($seatIdArray)); // số ghế đặt để kiểm tra/trừ lượt
            ...
            if (!empty($voucherCode)) {
                ...
                $result = $discountCodeModel->validateDiscountCode($voucherCode, $totalPrice, $movieId, $seatCount, $userId);
            ...
            try {
                ...
                // Trừ lượt sử dụng mã giảm giá (nếu có)
                if ($discountId) {
                    // Lượt dùng được tính dựa trên bookings, không cần trừ trực tiếp
                    $usageUpdated = $discountCodeModel->consumeUsage($discountId, $seatCount, $conn);
                    if (!$usageUpdated) {
                        $conn->rollBack();
                        ob_clean();
                        echo json_encode(['success' => false, 'message' => 'Mã giảm giá đã hết lượt sử dụng hoặc không đủ lượt cho số ghế đã chọn'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
                        exit;
                    }
                }
