...
// Chức năng: CRUD phim, phân trang, lọc theo thể loại/rạp
class Movie
{
    ...
    // Lấy tất cả phim với thông tin thể loại và rạp (có thể lọc theo rạp)
    public function all($cinemaId = null)
    {
        try {
            ...
            // Query SQL với LEFT JOIN để lấy tên thể loại
            $sql = "SELECT movies.*, 
    ...
    // Lấy phim với phân trang (có thể lọc theo rạp, trạng thái, tìm kiếm)
    ...
    // Lấy danh sách cơ bản để hiển thị trong dropdown (có thể chỉ lấy phim active)
    ...
    // Lưu danh sách rạp cho một phim (replace=true: thay thế toàn bộ, false: chỉ thêm mới)
    ...
    // Lấy phim đang chiếu (status=active, release_date <= today, end_date >= today)
    ...
    // Xóa phim (xóa cả rạp liên quan trong bảng trung gian)
