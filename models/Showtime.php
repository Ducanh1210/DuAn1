...
class Showtime
{
    ...
    /**
 * Lấy danh sách cinema/room cho showtime cùng movie và cùng ngày
 * Trả về mảng các showtime (showtime_id, start_time, price, room_id, room_name, room_code, capacity, cinema_id, cinema_name)
 */
public function getCinemasForShowtime($showtimeId)
{
    try {
        $sql = "SELECT 
                    s.id AS showtime_id,
                ...
                WHERE s.movie_id = (
                    SELECT movie_id FROM showtimes WHERE id = :id
                )
                  AND DATE(s.start_time) = DATE((
                    SELECT start_time FROM showtimes WHERE id = :id
                  ))
                ORDER BY c.name, r.name, s.start_time";
        $stmt = $this->conn->prepare($sql);
