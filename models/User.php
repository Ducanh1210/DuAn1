...
{
    try {
        // Lấy thông tin cũ để tránh lỗi khi form không gửi đủ dữ liệu
        $old = $this->find($id) ?: [];
        ...
        // Dùng giá trị mới nếu có, nếu không có dùng giá trị cũ từ DB
        $params = [
            ':id' => $id,
            ':full_name' => $data['full_name'] ?? ($old['full_name'] ?? null),
            ':email' => $data['email'] ?? ($old['email'] ?? null),
            ':phone' => $data['phone'] ?? ($old['phone'] ?? null),
            ':birth_date' => $data['birth_date'] ?? ($old['birth_date'] ?? null),
            ':tier_id' => $data['tier_id'] ?? ($old['tier_id'] ?? null),
            ':role' => $data['role'] ?? ($old['role'] ?? 'customer'),
            ':total_spending' => $data['total_spending'] ?? ($old['total_spending'] ?? 0.00)
        ];
