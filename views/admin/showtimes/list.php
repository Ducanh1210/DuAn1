            ...
            <?php if (!empty($data)): ?>
              ...
              <tr>
                ...
                <td>
                  ...
                  // Lấy ngày và giờ chiếu của lịch chiếu
                  $showDate = $item['show_date'] ? date('Y-m-d', strtotime($item['show_date'])) : null;
                  $endTime = $item['end_time'] ? date('H:i:s', strtotime($item['end_time'])) : null;
                  
                  // Kiểm tra trạng thái dựa trên ngày và giờ chiếu cụ thể
                  if ($showDate && $endTime) {
                    ...
                    elseif ($showDate == $currentDate) {
                      // Nếu giờ kết thúc đã qua → Hết hạn
                      if ($endTime < $currentTime) {
                        echo '<span class="badge bg-danger">Hết hạn</span>';
                      }
                      // Nếu giờ kết thúc chưa đến → Sắp chiếu
                      else {
                        echo '<span class="badge bg-warning text-dark">Sắp chiếu</span>';
                      }
                    }
                    // Nếu ngày chiếu là tương lai → Sắp chiếu
                    else {
                      echo '<span class="badge bg-warning text-dark">Sắp chiếu</span>';
                    }
                  } else {
