<?php
// SHOWTIMES/EDIT.PHP - TRANG SỬA LỊCH CHIẾU ADMIN
// Chức năng: Form sửa lịch chiếu (phim, rạp, phòng, ngày, giờ, loại)
// Biến từ controller: $showtime (thông tin lịch chiếu cần sửa), $errors (lỗi validation), $movies, $cinemas, $rooms
?>
<div class="container-fluid">
  <div class="card">
    ...
    <div class="card-body">
      ...
      <!-- Form sửa lịch chiếu: onsubmit gọi hàm validateShowtimeForm() để validate client-side -->
        ...
        <div class="row">
          <div class="col-md-6">
              ...
              <select name="movie_id" id="movie_id" class="form-select <?= !empty($errors['movie_id']) ? 'is-invalid' : '' ?>" >
                ...
                <?php if (!empty($movies)): ?>
                  <?php foreach ($movies as $movie): ?>
                    <!-- selected: đánh dấu phim đang được chọn (từ $_POST nếu đã submit, nếu không thì từ $showtime) -->
            ...
            <div class="mb-3">
              ...
              <select name="room_id" id="room_id" class="form-select <?= !empty($errors['room_id']) ? 'is-invalid' : '' ?>" <?= (empty($currentCinemaId) && empty($managerCinemaId)) ? 'disabled' : '' ?>>
                <option value="">-- Chọn phòng chiếu --</option>
                <?php if (!empty($rooms)): ?>
                  <?php foreach ($rooms as $room): ?>
                    <?php 
                      $isSelected = (isset($_POST['room_id']) ? $_POST['room_id'] : $showtime['room_id']) == $room['id'];
                      // Manager chỉ thấy phòng của rạp mình, Admin thấy phòng của rạp đã chọn
                      $targetCinemaId = !empty($managerCinemaId) ? $managerCinemaId : $currentCinemaId;
                      $isFromTargetCinema = $targetCinemaId && $room['cinema_id'] == $targetCinemaId;
                    ?>
                    <?php if ($isFromTargetCinema || (!$targetCinemaId && empty($managerCinemaId))): ?>
...
<script>
  ...
  const allRoomsData = [
    <?php if (!empty($rooms)): ?>
      <?php foreach ($rooms as $index => $room): ?>
        <?php 
          $targetCinemaId = !empty($managerCinemaId) ? $managerCinemaId : $currentCinemaId;
          $showRoom = empty($targetCinemaId) || $room['cinema_id'] == $targetCinemaId;
        ?>
        <?php if ($showRoom): ?>
        {
          id: <?= $room['id'] ?>,
  ...
  <?php else: ?>
    ...
    const initialCinemaId = cinemaSelect ? cinemaSelect.value : null;
    if (initialCinemaId) {
