  ...
  <div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
      <div>
        ...
        <?php if (isset($isStaff) && $isStaff && !empty($currentCinemaName)): ?>
        ...
        <div class="row">
          <div class="col-md-12 mb-3">
            <div class="d-flex flex-wrap gap-2 align-items-center">
              <a href="?act=contacts<?= $cinemaFilter ? '&cinema_id=' . $cinemaFilter : '' ?>" class="btn btn-sm <?= !$selectedStatus ? 'btn-primary' : 'btn-outline-primary' ?>">
                Tất cả (<?= array_sum($statusCounts) ?>)
              </a>
              <a href="?act=contacts&status=pending<?= $cinemaFilter ? '&cinema_id=' . $cinemaFilter : '' ?>" class="btn btn-sm <?= $selectedStatus == 'pending' ? 'btn-warning' : 'btn-outline-warning' ?>">
                Chờ xử lý (<?= $statusCounts['pending'] ?>)
              </a>
              <a href="?act=contacts&status=processing<?= $cinemaFilter ? '&cinema_id=' . $cinemaFilter : '' ?>" class="btn btn-sm <?= $selectedStatus == 'processing' ? 'btn-info' : 'btn-outline-info' ?>">
                Đang xử lý (<?= $statusCounts['processing'] ?>)
              </a>
              <a href="?act=contacts&status=resolved<?= $cinemaFilter ? '&cinema_id=' . $cinemaFilter : '' ?>" class="btn btn-sm <?= $selectedStatus == 'resolved' ? 'btn-success' : 'btn-outline-success' ?>">
                Đã xử lý (<?= $statusCounts['resolved'] ?>)
              </a>
              <a href="?act=contacts&status=closed<?= $cinemaFilter ? '&cinema_id=' . $cinemaFilter : '' ?>" class="btn btn-sm <?= $selectedStatus == 'closed' ? 'btn-secondary' : 'btn-outline-secondary' ?>">
          ...
          <div class="col-md-12">
            <form method="GET" action="" class="d-inline-flex gap-2 align-items-center">
              ...
              <?php endif; ?>
              <label for="cinema_filter" class="form-label mb-0">Lọc theo rạp:</label>
              <select name="cinema_id" id="cinema_filter" class="form-select form-select-sm" style="max-width: 250px;" onchange="this.form.submit()">
      ...
      <div class="table-responsive">
        <table class="table table-striped table-hover">
          ...
          <tbody>
            ...
            <?php else: ?>
              <tr>
                <td colspan="9" class="text-center text-muted py-4">Chưa có liên hệ nào</td>
              </tr>
            <?php endif; ?>
          </tbody>
        </table>
