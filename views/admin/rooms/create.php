<?php
// ROOMS/CREATE.PHP - TRANG TẠO PHÒNG CHIẾU MỚI ADMIN
// Chức năng: Form tạo phòng chiếu mới (chọn rạp, tên phòng, mã phòng, số hàng, số cột)
// Biến từ controller: $errors (lỗi validation), $cinemas (danh sách rạp)
?>
<div class="container-fluid">
  <div class="card">
    ...
    <div class="card-body">
      ...
      <!-- Form tạo phòng: onsubmit gọi hàm validateRoomForm() để validate client-side -->
      <form action="" method="post" id="roomForm" onsubmit="return validateRoomForm(event)">
              ...
              <select name="cinema_id" id="cinema_id" class="form-select <?= !empty($errors['cinema_id']) ? 'is-invalid' : '' ?>" >
                ...
                <?php if (!empty($cinemas)): ?>
                  <?php foreach ($cinemas as $cinema): ?>
                    <!-- selected: đánh dấu rạp đang được chọn (nếu đã submit form) -->
        ...
        <div class="mt-4">
          <button type="submit" class="btn btn-primary">
            <i class="bi bi-check-circle"></i> Thêm phòng
          </button>
          <a href="<?= BASE_URL ?>?act=rooms" class="btn btn-secondary">
...
<script>
  // Old validation - keeping for compatibility
