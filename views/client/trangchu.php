...
$promoBannerData = array_map(function ($promo) {
    ...
    return [
        ...
        'link' => !empty($promo['movie_id'])
            ? BASE_URL . '?act=lichchieu&movie_id=' . urlencode($promo['movie_id'])
            : BASE_URL . '?act=khuyenmai',
        'remaining_uses' => $promo['remaining_uses'] ?? null,
...
<script>
    ...
    (function() {
        ...
        const promoBanners = (window.HOMEPAGE_PROMOS || []).map(banner => ({
            ...
            remaining_uses: banner.remaining_uses ?? null,
            usage_limit: banner.usage_limit ?? null
        }));
        ...
        pages.forEach((pageBanners, pageIndex) => {
            ...
            pageBanners.forEach(banner => {
                ...
                if (banner.usage_limit !== null && banner.usage_limit !== undefined) {
                    const remainingEl = document.createElement('span');
                    remainingEl.className = 'promo-usage';
                    const remaining = banner.remaining_uses !== null && banner.remaining_uses !== undefined
                        ? banner.remaining_uses
                        : banner.usage_limit;
                    remainingEl.textContent = `Còn ${remaining}/${banner.usage_limit} lượt`;
