...
<script>
    ...
    function toggleSeat(seatElement) {
        ...
        if (seatElement.classList.contains('selected')) {
            ...
            if (groupSeats.length > 0) {
                ...
                remainingSeats = totalPeople - selectedSeats.length;
                // Enforce adjacent mode after a group is chosen
                enforceAdjacentModeAfterSelection();
                console.debug('after add group', {
                    selectedGroups,
    ...
    function removeSeatFromGroup(seatId) {
        ...
        for (let i = selectedGroups.length - 1; i >= 0; i--) {
            ...
            if (seatIndex !== -1) {
                ...
                refreshAdjacentOptionsUI();
                enforceAdjacentModeAfterSelection();
                console.debug('after remove group', {
                    selectedGroups,
                    selectedAdjacentCount,
                    remainingSeats,
                    selectedSeatsLength: selectedSeats.length
    ...
    function selectAdjacentSeatsSmart(startSeatElement, count, allowLastSingleSeat = false) {
        ...
        // Trường hợp chọn 3 hoặc 4 ghế: chỉ chọn trong block hiện tại và ôm sát về lề block
        if (count >= 3) {
            let validRanges = [];
            if (startBlock === 'left') {
                if (count === 3) {
    ...
    function updateTicketSelection() {
        ...
        const allowedForTotal = allowedMap[totalPeople] || [];

        // Render adjacent options via the centralized renderer so DOM is consistent
        refreshAdjacentOptionsUI(allowedForTotal);
    }
    ...
    function refreshAdjacentOptionsUI(allowedForTotal) {
        const adjacentOptions = document.getElementById('adjacentOptions');
        if (!adjacentOptions) return;

        const totalPeople = adultCount + studentCount;
        const remSeats = totalPeople - selectedSeats.length;
        // If no allowedForTotal provided, compute default mapping
        const allowedMapLocal = {
            1: [1],
            2: [2],
            3: [3],
            4: [2, 4],
        ...
        const allowed = (Array.isArray(allowedForTotal) && allowedForTotal.length > 0) ? allowedForTotal : (allowedMapLocal[totalPeople] || []);

        // If user has already selected a group, consider primary mode
        let primaryMode = null;
        if (selectedGroups.length > 0) {
            primaryMode = selectedGroups[0].count;
        }

        // Rebuild adjacent options DOM so state is deterministic
        adjacentOptions.innerHTML = '';
        const optionNodes = [];
        for (let count = 1; count <= 4; count++) {
            const opt = document.createElement('div');
            opt.className = 'adjacent-option';
            opt.setAttribute('data-count', count);
            ...
            opt.appendChild(seatsContainer);

            adjacentOptions.appendChild(opt);
        ...
        optionNodes.forEach(opt => {
            const count = parseInt(opt.getAttribute('data-count'), 10);

            // Enabled only if allowed by mapping and <= remaining seats
            let enabled = allowed.includes(count) && (remSeats > 0 && count <= remSeats);

            // If primaryMode exists and still fits, make only primary enabled
            if (primaryMode !== null && remSeats >= primaryMode) {
                enabled = (count === primaryMode);
            }

            if (enabled) {
                opt.classList.remove('disabled');
                enabledOptions.push(count);
            } else {
            ...
            opt.onclick = function() {
                ...
                document.querySelectorAll('.adjacent-option').forEach(o => o.classList.remove('active'));
                this.classList.add('active');
                const newAdjacentCount = parseInt(this.getAttribute('data-count'), 10);
                selectedAdjacentCount = newAdjacentCount;
                remainingSeats = totalPeople - selectedSeats.length;
                updateDisabledColumns();
                // After user click, ensure UI consistency
                refreshAdjacentOptionsUI();
                updateSummary();
            };
        });

        // Auto-switch logic: if current selectedAdjacentCount no longer fits, pick a fallback
        if (remSeats === 0) {
            document.querySelectorAll('.adjacent-option').forEach(o => o.classList.remove('active'));
            selectedAdjacentCount = 0;
            return;
        ...
        const uniqueEnabled = Array.from(new Set(enabledOptions)).sort((a, b) => a - b);

        if (selectedAdjacentCount > remSeats || selectedAdjacentCount === 0 || !uniqueEnabled.includes(selectedAdjacentCount)) {
            // choose the largest enabled option that is <= remSeats
            let pick = null;
            for (let i = uniqueEnabled.length - 1; i >= 0; i--) {
                if (uniqueEnabled[i] <= remSeats) {
                    pick = uniqueEnabled[i];
                    break;
                }
            }
            if (pick === null && uniqueEnabled.length > 0) pick = uniqueEnabled[0];

            if (pick !== null) {
                console.debug('refreshAdjacentOptionsUI pick', {
                    remSeats,
                    uniqueEnabled,
                    pick,
                    selectedAdjacentCountBefore: selectedAdjacentCount
                });
                selectedAdjacentCount = pick;
                document.querySelectorAll('.adjacent-option').forEach(o => {
                    const c = parseInt(o.getAttribute('data-count'), 10);
                    if (c === pick) o.classList.add('active');
                    else o.classList.remove('active');
                });
                console.debug('refreshAdjacentOptionsUI after', {
                    selectedAdjacentCount
                });
            }
        }
    ...
    function enforceAdjacentModeAfterSelection() {
        const totalPeople = adultCount + studentCount;
        ...
        const allowed = allowedMapLocal[totalPeople] || [];
        const remSeats = totalPeople - selectedSeats.length;

        // determine enabled options (allowed and <= remSeats)
        const enabled = allowed.filter(c => c <= remSeats);

        // prefer largest enabled (so 4->3 for remSeats=3)
        let pick = null;
        if (enabled.length > 0) {
            pick = enabled.sort((a, b) => b - a)[0];
        }

        // if nothing enabled, clear active
        if (pick === null) {
            document.querySelectorAll('.adjacent-option').forEach(o => o.classList.remove('active'));
            selectedAdjacentCount = 0;
            return;
        ...
        console.debug('enforceAdjacentModeAfterSelection', {
            totalPeople,
            remSeats,
            allowed,
            enabled,
        ...
        console.debug('enforceAdjacentModeAfterSelection after', {
            selectedAdjacentCount
        });
        // Ensure UI is consistent after enforcing mode
        try {
            refreshAdjacentOptionsUI();
        } catch (e) {
            console.debug('refreshAdjacentOptionsUI error', e);
        }
    }
