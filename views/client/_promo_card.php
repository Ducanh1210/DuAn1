...
$usageLabel = '';
if (!empty($promo['usage_label'])) {
    $usageLabel = htmlspecialchars($promo['usage_label']);
} else {
    $remaining = $promo['remaining_uses'] ?? null;
    $limit = $promo['usage_limit'] ?? null;
    if ($limit !== null) {
        $usageLabel = 'Còn ' . (int)$remaining . '/' . (int)$limit . ' lượt';
    }
}
$movieLink = $isMovieSpecific && !empty($promo['movie_id'])
    ? BASE_URL . '?act=lichchieu&movie_id=' . urlencode($promo['movie_id'])
    : (!empty($promo['cta_link']) ? htmlspecialchars($promo['cta_link']) : BASE_URL . '?act=khuyenmai');
?>
<article class="promo-card <?= $isMovieSpecific ? 'promo-card-with-movie' : '' ?> <?= !$isActive ? 'promo-card-inactive' : '' ?>" data-status="<?= $status ?>">
    <?php if ($isMovieSpecific && $movieImage): ?>
        <div class="promo-movie-image">
            <a href="<?= $movieLink ?>" title="<?= $movieTitle ? 'Xem lịch chiếu ' . $movieTitle : 'Xem lịch chiếu' ?>">
    ...
    <div class="promo-card-content">
        ...
        <div class="promo-actions">
            <?php if ($status === 'ongoing'): ?>
                <a class="btn-primary" href="<?= !empty($promo['cta_link']) ? htmlspecialchars($promo['cta_link']) : ($movieLink ?? BASE_URL . '?act=datve') ?>">
