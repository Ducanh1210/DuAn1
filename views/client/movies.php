...
.max-seats-note::before {
    ...
    function canAddSeatsWithoutGap(newSeats) {
        ...
        if (totalPeople >= 2) {
            ...
            if (selectedGroups.length > 0) {
                ...
                // Nếu nhóm cuối cùng đã chọn đủ số ghế theo selectedAdjacentCount hiện tại
                // HOẶC số ghế trong nhóm cuối khác với selectedAdjacentCount hiện tại (đã thay đổi option)
                if (lastGroup.seats.length >= selectedAdjacentCount || lastGroup.count !== selectedAdjacentCount) {
                    currentGroupComplete = true;
                }
            }

            // Nếu chưa có nhóm nào hoặc nhóm hiện tại chưa chọn đủ, kiểm tra xem có đang chọn liền nhau không
            // CHỈ kiểm tra cùng hàng khi đang chọn nhóm hiện tại chưa hoàn thành
            const isSelectingAdjacent = selectedAdjacentCount > 0 && remainingSeats > 0 && !currentGroupComplete;

            if (isSelectingAdjacent) {
                // Kiểm tra xem ghế mới có cùng hàng với nhóm đang chọn không
                // Lấy hàng của nhóm cuối cùng (nếu có)
                let lastGroupRow = null;
                if (selectedGroups.length > 0) {
                    const lastGroup = selectedGroups[selectedGroups.length - 1];
                    if (lastGroup.seats.length > 0) {
                        const firstSeatInGroup = lastGroup.seats[0];
                        const seatEl = document.querySelector(`[data-seat-id="${firstSeatInGroup.id}"]`);
                        if (seatEl) {
                            const row = seatEl.closest('.seat-row');
                            if (row) {
                                lastGroupRow = row.getAttribute('data-row-label');
                            }
                        }
                    }
                }

                // Nếu có nhóm đang chọn, kiểm tra ghế mới có cùng hàng không
                if (lastGroupRow) {
                    const newSeatsRows = new Set();
                    newSeats.forEach(seat => {
                        ...
                        if (seatEl) {
                            ...
                            if (row) {
                                ...
                                if (rowLabel) {
                                    newSeatsRows.add(rowLabel);
                                }
                            }
                        }
                    });

                    // Nếu ghế mới không cùng hàng với nhóm đang chọn, không cho phép
                    if (!newSeatsRows.has(lastGroupRow)) {
                        return false;
                    }
                }
            }
    ...
    function updateTicketSelection() {
        ...
        if (adultCount !== prevAdult || studentCount !== prevStudent) {
            ...
            // Nếu số lượng người không thay đổi và đã có selectedAdjacentCount
            // Kiểm tra xem nhóm cuối cùng đã hoàn thành chưa
            let shouldKeepOptions = false;
            if (selectedAdjacentCount > 0 && selectedGroups.length > 0) {
                const lastGroup = selectedGroups[selectedGroups.length - 1];
                // Nếu nhóm cuối đã hoàn thành (đã chọn đủ số ghế), cho phép render lại options
                // để người dùng có thể chọn số lượng khác cho nhóm tiếp theo
                if (lastGroup.seats.length >= lastGroup.count && remainingSeats > 0) {
                    // Nhóm đã hoàn thành, render lại options để chọn nhóm mới
                    shouldKeepOptions = false;
                } else {
                    // Nhóm chưa hoàn thành, giữ lại options hiện tại
                    shouldKeepOptions = true;
                }
            } else if (selectedAdjacentCount > 0 && selectedSeats.length === 0) {
                // Chưa chọn ghế nào, giữ lại options
                shouldKeepOptions = true;
            }
        ...
        // Luôn chỉ hiển thị tối đa 3 options: 2, 3, 4 (không hiển thị 5, 6, 7, 8...)
        // Option 1 chỉ xuất hiện khi số lượng = 1
        // Với 3 người, chỉ hiển thị 3 (không hiển thị 2)
        // Với 4 người, chỉ hiển thị 2, 4 (không hiển thị 3)
        // Với 5 người, chỉ hiển thị 2, 3 (không hiển thị 4)
        if (totalPeople === 1) {
            availableOptions = [1];
            selectedAdjacentCount = 1;
            updateDisabledColumns();
        } else if (totalPeople === 3) {
            // 3 người: chỉ hiển thị 3 (không hiển thị 2)
            availableOptions = [3];
            selectedAdjacentCount = 3;
        } else if (totalPeople === 4) {
            // 4 người: chỉ hiển thị 2, 4 (không hiển thị 3)
            const allOptions = [2, 4];
            availableOptions = allOptions.filter(opt => opt <= remainingSeats);
            
            // Nếu không có option nào phù hợp, dùng remainingSeats
            if (availableOptions.length === 0 && remainingSeats > 0) {
                availableOptions = [remainingSeats];
            }
            
            // Mặc định chọn option lớn nhất không vượt quá remainingSeats
            if (selectedAdjacentCount === 0 || selectedAdjacentCount > remainingSeats) {
                if (availableOptions.length > 0) {
                    // Chọn option lớn nhất
                    selectedAdjacentCount = Math.max(...availableOptions);
                } else if (remainingSeats > 0) {
                    selectedAdjacentCount = remainingSeats;
                }
            }
        } else if (totalPeople === 5) {
            // 5 người: chỉ hiển thị 2, 3 (không hiển thị 4)
            const allOptions = [2, 3];
            availableOptions = allOptions.filter(opt => opt <= remainingSeats);
            ...
            // Với số lượng >= 2 (trừ 3, 4, 5), luôn chỉ hiển thị options 2, 3, 4 (tối đa)
            // Lọc các options phù hợp với số ghế còn lại
            const allOptions = [2, 3, 4];
            availableOptions = allOptions.filter(opt => opt <= remainingSeats);
        ...
        // Clear container trước khi render options mới (đảm bảo không có options cũ)
        adjacentOptions.innerHTML = '';
        
        // Xác định option nào sẽ được chọn (chỉ 1 option duy nhất)
        let optionToActivate = null;
        
        // Nếu selectedAdjacentCount hợp lệ và có trong availableOptions, dùng nó
        if (selectedAdjacentCount > 0 && selectedAdjacentCount <= remainingSeats && availableOptions.includes(selectedAdjacentCount)) {
            optionToActivate = selectedAdjacentCount;
        } else {
            // Nếu không, chọn option lớn nhất
            if (availableOptions.length > 0) {
                optionToActivate = Math.max(...availableOptions);
                selectedAdjacentCount = optionToActivate;
            }
        }
        ...
        availableOptions.forEach(count => {
            ...
            // Chỉ active option được chọn (chỉ 1 option duy nhất)
            if (count === optionToActivate) {
            ...
            option.onclick = function() {
                ...
                // Kiểm tra xem nhóm cuối cùng đã hoàn thành chưa
                // Nếu đã hoàn thành, cho phép thay đổi selectedAdjacentCount cho nhóm tiếp theo
                let canChangeOption = true;
                if (selectedGroups.length > 0) {
                    const lastGroup = selectedGroups[selectedGroups.length - 1];
                    // Nếu nhóm cuối chưa hoàn thành và đang thay đổi option, không cho phép
                    if (lastGroup.seats.length < lastGroup.count) {
                        canChangeOption = false;
                        alert('Vui lòng hoàn thành nhóm hiện tại trước khi thay đổi số lượng ghế!');
                        // Khôi phục lại option cũ
                        const oldOption = document.querySelector(`.adjacent-option[data-count="${selectedAdjacentCount}"]`);
                        if (oldOption) {
                            activateOption(oldOption, true);
                        }
                        return;
                    }
                }
    ...
    function updateAdjacentOptionsAfterSelection() {
        ...
        if (selectedGroups.length > 0) {
            const lastGroup = selectedGroups[selectedGroups.length - 1];
            // Nếu nhóm cuối đã chọn đủ số ghế, cho phép chọn nhóm mới với số lượng khác
            if (lastGroup.seats.length >= lastGroup.count && remainingSeats > 0) {
                // Cập nhật lại options để cho phép chọn số lượng khác cho nhóm tiếp theo
                updateTicketSelection();
            }
        }
...
<section class="reviews-section" style="background: #1a1a1a; padding: 20px 0; margin-top: 30px;">
    <style>
        ...
        [data-theme="light"] .review-textarea-input {
            background: #fff !important;
            border-color: #ddd !important;
            color: #000 !important;
        }
        [data-theme="light"] .review-textarea-input::placeholder {
            color: rgba(0, 0, 0, 0.5) !important;
        }
        [data-theme="light"] .review-char-count {
            color: rgba(0, 0, 0, 0.6) !important;
        }
        [data-theme="light"] .review-rating-text {
            color: rgba(0, 0, 0, 0.6) !important;
        }
        [data-theme="light"] .review-form-container {
            background: rgba(255, 255, 255, 0.9) !important;
        }
        [data-theme="light"] .review-form-container div[style*="background: rgba(255, 193, 7"] span {
            color: rgba(0, 0, 0, 0.8) !important;
        }
        [data-theme="light"] .review-form-container div[style*="background: rgba(76, 175, 80"] {
            color: rgba(0, 0, 0, 0.9) !important;
        }
        [data-theme="light"] .review-form-container div[style*="background: rgba(76, 175, 80"] span {
            color: rgba(0, 0, 0, 0.9) !important;
        }
    </style>
    <?php if (empty($isComingSoon)): ?>
    <div class="container">
        <h2 class="review-form-title" style="color: #fff; font-size: 18px; font-weight: 600; margin-bottom: 15px; text-align: center;">
            ...
            <div class="review-form-container" style="background: rgba(255, 255, 255, 0.05); border-radius: 8px; padding: 15px; margin-bottom: 20px; position: relative;">
                ...
                <form action="<?= BASE_URL ?>?act=submit-movie-review" method="POST" id="reviewForm">
                    ...
                    <div style="margin-bottom: 12px;">
                        <label class="review-label" style="color: #fff; display: block; margin-bottom: 6px; font-weight: 500; font-size: 13px;">Xếp hạng <span style="color: #ff8c00;">*</span></label>
                        <div class="star-rating-input" style="display: flex; gap: 5px; align-items: center; <?= (!$isLoggedIn || !$hasPurchased) ? 'opacity: 0.5; pointer-events: none;' : '' ?>">
                            ...
                            <?php endfor; ?>
                            <span id="ratingText" class="review-rating-text" style="color: rgba(255, 255, 255, 0.7); margin-left: 10px; font-size: 12px;">
                            ...
                            <style>
                                ...
                                [data-theme="dark"] .review-rating-text,
                                :not([data-theme]) .review-rating-text {
                    ...
                    <div style="margin-bottom: 12px;">
                        <label for="reviewContent" class="review-label" style="color: #fff; display: block; margin-bottom: 6px; font-weight: 500; font-size: 13px;">
                            Bình luận <span style="color: #ff8c00;">*</span>
                        </label>
                        <textarea name="content" id="reviewContent" class="review-textarea-input" rows="4"
                        ...
                        <div class="review-char-count" style="text-align: right; margin-top: 3px; color: rgba(255, 255, 255, 0.5); font-size: 11px;">
